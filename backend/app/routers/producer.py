...
def _get_demo_scripts(project_id: Optional[str] = None):
    """内置 Demo 剧本"""
    if project_id == "proj-fuhua":
        return [
            {
                "id": "fuhua-ep1", "title": "第一集 · 猎物与猎手的名利场", "episode": 1, "branch": None, "status": "published",
                "roles": [
                    {"name": "顾晚", "desc": "破产千金，外表柔弱，内心冷静，带着复仇目标闯入名利场"},
                    {"name": "陆时谦", "desc": "商业死神，优雅危险，习惯把所有人纳入自己的棋局"},
                    {"name": "宋秘书", "desc": "陆时谦的执行者，沉默、精准，负责收网"},
                ],
                "scenes": [
                    {"id": 1, "visual": "私人晚宴厅，水晶灯映着香槟塔。顾晚穿红色礼服穿过人群，目光锁定陆时谦手中的极光之泪。", "audio": "顾晚旁白：今晚，我只需要拿回顾家失去的钥匙。"},
                    {"id": 2, "visual": "露台上，陆时谦亲手把极光之泪戴到顾晚颈间。镜头贴近项链冷光与顾晚克制的呼吸。", "audio": "陆时谦：顾小姐，你看起来不像来赴宴，倒像来狩猎。"},
                    {"id": 3, "visual": "陆时谦书房，顾晚借项链折射出的指纹打开保险柜，黑色文件袋露出顾氏并购案封面。", "audio": "保险柜轻响，顾晚低声：终于找到你了。"},
                    {"id": 4, "visual": "灯光骤亮，宋秘书站在门口，陆时谦从阴影中走出。顾晚手里的文件被迫停在半空。", "audio": "陆时谦：猎物进笼之后，才知道谁在等谁。"},
                ],
            },
            {
                "id": "fuhua-ep2a", "title": "第二集A · 带刺的玫瑰", "episode": 2, "branch": "A", "status": "published",
                "roles": [
                    {"name": "顾晚", "desc": "宁为玉碎，选择用危险换取逃生窗口"},
                    {"name": "陆时谦", "desc": "被短暂反制后仍保持冷静的掌控者"},
                ],
                "scenes": [
                    {"id": 1, "visual": "书房内，顾晚突然抓起黄铜裁纸刀，刀锋抵住陆时谦的领口，窗外闪电照亮两人的侧脸。", "audio": "顾晚：让他们退后，否则今晚谁都别想体面收场。"},
                    {"id": 2, "visual": "宋秘书缓慢放下对讲机，陆时谦却微笑着靠近刀锋，危险感被压到静默的极限。", "audio": "陆时谦：你拿刀的手在抖，顾晚。"},
                    {"id": 3, "visual": "地下禁闭室，顾晚从旧手机里翻出残缺录音，又在墙后发现建筑管道图。", "audio": "录音杂音中传出：顾氏的账，不该只有一个人背。"},
                ],
            },
            {
                "id": "fuhua-ep2b", "title": "第二集B · 完美的金丝雀", "episode": 2, "branch": "B", "status": "published",
                "roles": [
                    {"name": "顾晚", "desc": "选择伪装顺从，用更长的蛰伏换取核心证据"},
                    {"name": "陆时谦", "desc": "享受控制感，同时被顾晚的反常安静吸引"},
                ],
                "scenes": [
                    {"id": 1, "visual": "顾晚在书房地毯上缓慢跪下，垂眸摘下项链，主动把极光之泪放回陆时谦掌心。", "audio": "顾晚：我输了。你想让我怎么做？"},
                    {"id": 2, "visual": "清晨办公室，顾晚端着咖啡经过陆时谦身侧，袖口里的微型镜头扫过加密文件。", "audio": "打印机轻响，咖啡杯落桌声掩盖了扫描提示音。"},
                    {"id": 3, "visual": "顾晚独自在洗手间隔间里拆开口红外壳，存储卡闪烁蓝光，文件名显示顾氏清算底稿。", "audio": "顾晚低声：金丝雀也可以记住笼子的锁孔。"},
                ],
            },
            {
                "id": "fuhua-ep2c", "title": "第二集C · 恶女的筹码", "episode": 2, "branch": "C", "status": "published",
                "roles": [
                    {"name": "顾晚", "desc": "放弃逃跑，直接用隐藏账户和证据与陆时谦谈判"},
                    {"name": "陆时谦", "desc": "第一次遇到能把筹码摆上牌桌的对手"},
                ],
                "scenes": [
                    {"id": 1, "visual": "顾晚把文件袋推回桌面，平静报出一串海外账户密码，陆时谦的笑意终于停住。", "audio": "顾晚：你要顾家的秘密，我要害死我父亲的人。我们可以成交。"},
                    {"id": 2, "visual": "电子屏上弹出多份并购流水和匿名转账记录，红色标记逐一连向董事会成员。", "audio": "系统提示音连续响起，空气里只剩键盘敲击声。"},
                    {"id": 3, "visual": "陆时谦走到顾晚面前，两人隔着一张合约对视。镜头俯拍，像棋盘上两枚终于相遇的棋子。", "audio": "陆时谦：从现在开始，你不是猎物，是合伙人。"},
                ],
            },
        ]
    if project_id and project_id != "demo-proj-002":
        return []
    return [
        {
